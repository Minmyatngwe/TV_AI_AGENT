...
# ----------------------------
def load_css():
    css_path = Path(__file__).resolve().parents[1] / "styles" / "tvdisplay.css"
    if css_path.exists():
...
with right_col:
    ...
    with preview_wrapper:
        ...
        if selected is None:
            ...
            with top_b:
                st.metric("Display Mode", "16:9 TV")
            ...
            with action_col3:
                st.download_button(
                    ...
                    data=f"Layout: {selected['display_name']}\nProject: {selected['project']}\nPath: {selected['path']}",
                    file_name="selected_layout_info.txt",
                    use_container_width=True,
                )
